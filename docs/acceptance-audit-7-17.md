...
| #13 synchronized stereo surface   | Open                        | In-app browser verifies session routing, deep-linked timestamp state, explicit unsupported-media state, and independent controls.                                                  | Current live fixture exposes one camera and one IMU stream, not two video streams; add a playable two-camera browser fixture and automated synchronization workflow.         |
| #14 sensor timeline/accessibility | Open                        | In-app browser verifies keyboard seek, named controls/images, no duplicate IDs, Canvas sensor rendering, and visible gap/fallback states.                                          | Add pose track, zoom, loop/rate controls, visible-window/prefetch cancellation evidence, automated accessibility scan, and gap snapshots.                                    |
| #15 playback metrics              | Open                        | Diagnostics export schema, stable definitions, resource observations, and browser-visible metrics are implemented.                                                                 | Run and retain the declared deterministic 20-minute browser benchmark; record warm-seek, dropped-frame, drift, transfer, and bounded-memory evidence.                        |

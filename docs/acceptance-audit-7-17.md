...
| #10 TypeScript SDK/CLI            | Verified | Eleven SDK tests, the reference CLI, and the clean packed-consumer gate pass without monorepo resolution. Bigint timestamps, retries, cancellation, pagination, generated/reference API parity, migration policy, and package contents are covered.                | None found.                                                                   |
...
| #15 playback metrics              | Verified | Retained 20-minute Chromium report contains 240 resource samples, 23,262 observed presentations, warm-seek/drift/transfer/long-task/heap metrics, identity, and passing budgets.                                                                                   | None found; real codec decode and bounded queue behavior are verified by #16. |
| #16 WebCodecs path                | Verified | Capability-selected IVF demuxing runs in a generation-aware worker and presents decoded `VideoFrame`s through a bounded canvas queue. Browser tests encode real VP8 input, exercise seek storms and worker cleanup, and retain the labeled HTML fallback.          | None found.                                                                   |
| #17 scalable sensor rendering     | Verified | Prefetched Arrow IPC windows preserve int64/nulls off-thread, use an eight-window LRU with telemetry, compute three-axis pixel envelopes in a worker, and render virtualized tracks. The million-sample unit gate and retained 200,000-sample browser report pass. | None found.                                                                   |

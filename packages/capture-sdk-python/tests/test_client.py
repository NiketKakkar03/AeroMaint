...
class CaptureClientTests(unittest.TestCase):
    def test_sessions_pagination_bigints_and_auth(self):
        opener = QueueOpener(
            response(
                {
                    "items": [
                        {"id": "a", "start_ns": "9007199254740993", "end_ns": "9007199254740994"}
                    ],
                    "next_cursor": "c2",
                }
            ),
        ...
        client = CaptureClient(
            "https://api.test/",
            token="test-auth-token",  # noqa: S106
            opener=opener,
        )
    ...
    def test_typed_imu_window_preserves_extra_values(self):
                    ...
                    "items": [
                        {
                            ...
                            "values": {"ax": 1, "ay": 2.5, "az": "3", "temperature": 4},
                        }
                    ],
        ...
        window = CaptureClient("https://api.test", opener=opener).get_imu_window(
            "s/1", "imu/main", start_ns=1, end_ns=2
        )
        self.assertEqual(window.start_ns, 9_007_199_254_740_993)
        self.assertEqual(
            (window.samples[0].ax, window.samples[0].ay, window.samples[0].az), (1.0, 2.5, 3.0)
        )
    ...
    def test_retries_transport_and_retryable_http(self):
        http = HTTPError(
            ...
            "busy",
            {"Retry-After": "0"},
            io.BytesIO(b'{"code":"BUSY","detail":"try later","request_id":"r1"}'),
        )
        opener = QueueOpener(http, URLError(TimeoutError("slow")), response({"items": []}))
        page = CaptureClient(
            "https://api.test", opener=opener, backoff=0, max_attempts=3
        ).list_sessions()
    ...
    def test_structured_non_retryable_error(self):
        error = HTTPError(
            ...
            "missing",
            {},
            io.BytesIO(b'{"code":"NOT_FOUND","detail":"gone","trace_id":"t1"}'),
        )
        with self.assertRaises(CaptureHttpError) as caught:
            CaptureClient("https://api.test", opener=QueueOpener(error)).list_sessions()
        self.assertEqual(
            (caught.exception.status, caught.exception.code, caught.exception.problem.trace_id),
            (404, "NOT_FOUND", "t1"),
        )
    ...
    def test_invalid_imu_is_a_typed_response_error(self):
        client = CaptureClient(
            "https://api.test",
            opener=QueueOpener(response({"items": [{"timestamp_ns": "1", "values": {"ax": 1}}]})),
        )

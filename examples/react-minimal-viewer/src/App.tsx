...
export function MinimalViewer({ baseUrl, token }: MinimalViewerProps) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      for await (const item of client.iterateSessions({ signal: abort.signal }))
        items.push(item);
      setSessions(items);
  ...
  useEffect(() => {
    ...
    void (async () => {
      const next = await client.getManifest(sessionId, {
        signal: abort.signal
      });
      setManifest(next);
      const videoStream = next.streams.find(
        (stream) => stream.kind === "video"
      );
      const artifactId = videoStream?.artifactIds[0];
      if (artifactId !== undefined) {
        objectUrl = URL.createObjectURL(
          await client.getMediaArtifact(sessionId, artifactId, {
            signal: abort.signal
          })
        );
  ...
  const refreshImu = async () => {
    ...
    const videoStream = manifest?.streams.find(
      (stream) => stream.kind === "video"
    );
    if (!imuStream || !videoStream) return;
    const playhead =
      videoStream.startNs +
      BigInt(Math.round((video.current?.currentTime ?? 0) * 1e9));
    const startNs = playhead < imuStream.startNs ? imuStream.startNs : playhead;
    const endNs =
      startNs + 1_000_000_000n < imuStream.endNs
        ? startNs + 1_000_000_000n
        : imuStream.endNs;
    if (endNs <= startNs) return;
    const range = await client.getSampleRange(sessionId, imuStream.id, {
      startNs,
      endNs,
      format: "json"
    });
  ...
  return (
    <main>
      ...
      <video
        ...
        controls
        onTimeUpdate={() => {
          void refreshImu().catch((reason: unknown) =>
      ...
      <button
        type="button"
        onClick={() => {
          void exportWindow().catch((reason: unknown) =>
            setError(message(reason))
          );
        }}
      >
      ...
      {job && (
        <p>
          Export {job.id}: {job.status} ({Math.round(job.progress * 100)}%)
        </p>
      )}
      {job?.manifestUrl && (
        <a href={new URL(job.manifestUrl, baseUrl).toString()}>
...
function message(reason: unknown): string {
  return reason instanceof CaptureSdkError
